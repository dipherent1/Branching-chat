...
export async function POST(req: Request) {
  try {
    ...
    if (apiProvider === "gemini" && geminiApiKey) {
      ...
      const cleanModel = modelName?.replace(/^google\//, "") || "gemini-2.0-flash"
      model = google(cleanModel)
    } else {
      // Default: Vercel AI Gateway (pass model string directly)
      model = modelName || "google/gemini-2.0-flash"
    }
...
function createMockStreamResponse(
  ...
  console.log("[v0] Mock API received messages:", messages.map(m => ({ role: m.role, contentPreview: m.content.slice(0, 50) })))
